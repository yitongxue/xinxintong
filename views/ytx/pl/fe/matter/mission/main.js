...
app.controller('ctrlSetting', ['$scope', 'http2', '$modal', 'mediagallery', function($scope, http2, $modal, mediagallery) {
	var modifiedData = {};
	...
	$scope.addEnroll = function(assignedScenario) {
		$modal.open({
			...
			switch (data.scenario.name) {
				case 'registration':
					config.prototype.title = $scope.editing.title + '-' + '在线报名';
					break;
				case 'voting':
					config.prototype.title = $scope.editing.title + '-' + '在线投票';
					break;
				default:
			}
