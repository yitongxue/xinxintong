define(['require', 'page'], function(require, pageLib) {
	...
	ngApp.config(['$controllerProvider', '$routeProvider', '$locationProvider', '$compileProvider', function($controllerProvider, $routeProvider, $locationProvider, $compileProvider) {
		...
		$routeProvider.when('/rest/pl/fe/matter/signin/page', {
			templateUrl: '/views/ytx/pl/fe/matter/signin/page.html?_=4',
			controller: 'ctrlPage',
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					require(['/views/ytx/pl/fe/matter/signin/page.js'], function() {
						defer.resolve();
					});
			...
			templateUrl: '/views/ytx/pl/fe/matter/signin/record.html?_=4',
			controller: 'ctrlRecord',
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					require(['/views/ytx/pl/fe/matter/signin/record.js'], function() {
						defer.resolve();
					});
					return defer.promise;
				}
			...
			templateUrl: '/views/ytx/pl/fe/matter/signin/stat.html?_=1',
			controller: 'ctrlStat',
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					(function() {
						$.getScript('/views/ytx/pl/fe/matter/signin/stat.js', function() {
							defer.resolve();
						});
					})();
					return defer.promise;
			...
			templateUrl: '/views/ytx/pl/fe/matter/signin/coin.html?_=1',
			controller: 'ctrlCoin',
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					(function() {
						$.getScript('/views/ytx/pl/fe/matter/signin/coin.js', function() {
							defer.resolve();
						});
					})();
					return defer.promise;
			...
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					require(['/views/ytx/pl/fe/matter/signin/publish.js'], function() {
						defer.resolve();
					});
					return defer.promise;
				}
			...
			templateUrl: '/views/ytx/pl/fe/matter/signin/event.html?_=6',
			controller: 'ctrlEvent',
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					require(['/views/ytx/pl/fe/matter/signin/event.js'], function() {
						defer.resolve();
					});
					return defer.promise;
				}
			...
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					require(['/views/ytx/pl/fe/matter/signin/app.js'], function() {
						defer.resolve();
					});
					return defer.promise;
				}
	...
	ngApp.controller('ctrlFrame', ['$scope', '$location', '$uibModal', '$q', 'http2', function($scope, $location, $uibModal, $q, http2) {
		...
		$scope.createPage = function() {
			...
			$uibModal.open({
				templateUrl: '/views/ytx/pl/fe/matter/signin/component/createPage.html?_=3',
				backdrop: 'static',
