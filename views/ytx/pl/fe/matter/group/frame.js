	...
	ngApp.config(['$controllerProvider', '$routeProvider', '$locationProvider', function($controllerProvider, $routeProvider, $locationProvider) {
		...
		$routeProvider.when('/rest/pl/fe/matter/group/player', {
			...
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					require(['/views/ytx/pl/fe/matter/group/player.js'], function() {
						defer.resolve();
					});
			...
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					require(['/views/ytx/pl/fe/matter/group/running.js'], function() {
						defer.resolve();
					});
					return defer.promise;
				}
			...
			resolve: {
				load: function($q) {
					var defer = $q.defer();
					require(['/views/ytx/pl/fe/matter/group/setting.js'], function() {
						defer.resolve();
					});
					return defer.promise;
				}
